...
今回、条件検査、日別台帳、1レースの取得・予測・結果評価、欠測集計を実装した。9時・17時の定期確認は設定済み。発走時刻に合わせた取得には、開催日ごとにローカルrunnerの起動と稼働確認が必要。
...
開催前の準備を完了。日別台帳・欠測を照合して保存済み予測をまとめる[全期間集計手順](phase1-summary-operations.md)と[終了報告テンプレート](phase1-report-template.md)を使用する。集計処理の成功とPhase完了を区別し、公式台帳・時計同期の証拠を別途確認する。

## 2026-09-23の公式情報確認

JRAの[9月26日競馬番組](https://www.jra.go.jp/keiba/calendar2026/2026/9/0926.html)と[9月27日競馬番組](https://www.jra.go.jp/keiba/calendar2026/2026/9/0927.html)を確認。両ページは事前の予定で、正式出馬表の情報は通常木曜日16時頃に公開され、出馬投票や天候でレース番号・時刻などが変わり得ると明記している。予定表の時刻を正式台帳には転記しない。9/26・27の正式台帳は9/24の17時定期確認で出馬表公開を確認した後に作成・競合検査する。10/3・4は次週の正式発表を待つ。今回取得・予測・評価は未実行。
