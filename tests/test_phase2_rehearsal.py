...
from shibata.phase2_rehearsal import feature_frame, run, synthetic_raw
...
def test_tree_saved_prediction_matches_fit_and_unknown_category(name, tmp_path):
    ...
    train = feature_frame(synthetic_raw()).query("race_id.str.startswith('20260101') or race_id.str.startswith('20260201')")
...
def test_tree_fit_rejects_invalid_or_future_training_rows(change):
    config = json.loads(CONFIG.read_text(encoding='utf-8-sig'))
    frame = feature_frame(synthetic_raw()).query("race_id.str.startswith('20260101') or race_id.str.startswith('20260201')").copy()
...
def test_three_models_exact_same_market_population_and_no_test(tmp_path):
    ...
    for name in ('synthetic-raw.json', 'synthetic-features.csv'):
